...
def draw_wheel_rotation_indicator(surface, center, angular_velocity, theta, wheel_angle, max_speed=MAX_REQUEST_V):
    """
    Draw an arrow indicating wheel rotation direction and speed.
    The arrow length is proportional to the wheel speed.
    wheel_angle: angle of the wheel relative to robot frame (±45° for mecanum wheels)
    """
    ...
    # Calculate arrow direction based on wheel angle and robot orientation
    total_angle = theta + wheel_angle
    
    # Calculate arrow end points based on direction
    if angular_velocity > 0:
        angle = total_angle
    else:
        angle = total_angle + math.pi
        
    # Calculate arrow end points
    end_x = center[0] + arrow_length * math.cos(angle)
    end_y = center[1] - arrow_length * math.sin(angle)
    ...
    # Draw arrow head
    head_length = min(8, arrow_length/3)
    if arrow_length > head_length * 2:
        angle_head = math.pi/6
        if angular_velocity > 0:
        ...
        pygame.draw.lines(surface, color, False, [(end_x, end_y), (head1_x, head1_y)], 2)
        pygame.draw.lines(surface, color, False, [(end_x, end_y), (head2_x, head2_y)], 2)

def draw_robot(surface, pose, wheels_angular, requested, params):
    """
    Draw the robot as a single block with integrated wheels.
    """
    # Initialize font
    ...
    # Robot rectangle corners (main body)
    L = params['length']
    W = params['width']
    corners = [
        ( L/2,  W/2),
    ...
    # Draw integrated wheels (as colored sections at corners)
    wheel_size = 12  # Size of wheel indicators
    wheel_positions = [
        ( L/2,  W/2, math.pi/4),    # Front-left, 45°
        ( L/2, -W/2, -math.pi/4),   # Front-right, -45°
        (-L/2, -W/2, math.pi/4),    # Back-right, 45°
        (-L/2,  W/2, -math.pi/4),   # Back-left, -45°
    ]
    
    for (wx_loc, wy_loc, wheel_angle), w_ang in zip(wheel_positions, wheels_angular):
        # Calculate wheel center in world coordinates
        wx_world = x + (wx_loc * math.cos(theta) - wy_loc * math.sin(theta))
        wy_world = y + (wx_loc * math.sin(theta) + wy_loc * math.cos(theta))
        wheel_center = meters_to_pixels(wx_world, wy_world, screen_center)
        
        # Draw wheel as a filled circle
        pygame.draw.circle(surface, COLOR_WHEEL, wheel_center, wheel_size)
        
        # Draw rotation indicator with correct angle
        draw_wheel_rotation_indicator(surface, wheel_center, w_ang, theta, wheel_angle)
